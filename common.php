...
if (!file_exists('online.cache') || filemtime('online.cache') + $onlinecache < time()) {
	...
	if ($online) {
		...
		if (strlen($raw) < 2) {
			// Handle the error appropriately, e.g., set $chars to a default value or return an error response
			$chars = 0; // Default value, or handle the error as needed
		} else {
			$chars = Number(ord($raw[0]), ord($raw[1]));
			$raw = substr($raw, 2); // Number of characters
		}
